...
plot_enrichments <- function(enrichments_df = NULL, log_transform = TRUE,
  ...
  ## melt, log_transform and join with metadata before plotting
  ...
  ## check color pals within `plot_type` condition

  if(plot_type == "separate"){

    # average of each region from all samples of a group
    ...
    plt <- plt + theme_classic(base_size = 14)+
      ylab("normalized read density")+xlab("")+
    ...
    ## summarise based on 3 features in this case
    enrichments_mlt_jn_avg <- enrichments_jn %>%
